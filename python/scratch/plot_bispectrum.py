...
#    B = np.load(opj(test_dir, 'test{}.npy'.format(typestr)))
    B = np.load(opj(test_dir, 'bispectrum{}.npy'.format(typestr)))
    print 'loaded'
    #B = np.load(opj(test_dir, 'bispectrum.npy'))
    num_pass = np.load(opj(test_dir, 'num_pass{}.npy'.format(typestr)))
    #num_pass = num_pass[:-1,:-1,:-1]

    # For plotting, divide out num pass, 
    B[num_pass != 0,:] /= num_pass[num_pass != 0,np.newaxis]

    bins = np.load(opj(test_dir, 'bins{}.npy'.format(typestr)))
    ...
    fig.savefig(opj(test_dir, 'griddata{}.png'.format(typestr)))
    fig_r.savefig(opj(test_dir, 'griddata_raw{}.png'.format(typestr)))
    fig_s.savefig(opj(test_dir, 'griddata_raw_scatter{}.png'.format(typestr)))
    fig_d.savefig(opj(test_dir, 'griddata_1d{}.png'.format(typestr)))
